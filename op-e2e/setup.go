...
type System struct {
	...
	l2OutputSubmitter          *proposer.L2OutputSubmitter
	sequencerHistoryDBFileName string
	batchSubmitter             *batcher.BatchSubmitter
...
func (cfg SystemConfig) start() (*System, error) {
	...
	// Create the batcher and set it's config here because it needs to be derived from the accounts
	batcherPrivKey, err := wallet.PrivateKey(accounts.Account{
		URL: accounts.URL{
	...
	batchSubmitterAddr := crypto.PubkeyToAddress(batcherPrivKey.PublicKey)
	...
	for name, nodeConfig := range cfg.Nodes {
		...
		if c.Sequencer {
			c.SubmitterPrivKey = batcherPrivKey
		}
		if p, ok := p2pNodes[name]; ok {
			c.P2P = p
		}
	...
	// L2Output Submitter
	sys.l2OutputSubmitter, err = proposer.NewL2OutputSubmitter(proposer.Config{
	...
	sequencerHistoryDBFile, err := ioutil.TempFile("", "op-batcher.*.json")
	if err != nil {
	...
	// Batch Submitter
	sys.batchSubmitter, err = batcher.NewBatchSubmitter(batcher.Config{
		L1EthRpc:                   "ws://127.0.0.1:9090",
		L2EthRpc:                   sys.cfg.Nodes["sequencer"].L2NodeAddr,
		RollupRpc:                  rollupEndpoint,
		MinL1TxSize:                1,
