...
func (d *outputImpl) newBlock(ctx context.Context, l2Finalized eth.BlockID, l2Parent eth.BlockID, l2Safe eth.BlockID, l1Origin eth.BlockID, includeDeposits bool) (eth.BlockID, *derive.BatchData, error) {
	...
	payload, err := d.addBlock(ctx, fc, attrs, false)
	if err != nil {
...
func (d *outputImpl) step(ctx context.Context, l2Head eth.BlockID, l2Finalized eth.BlockID, unsafeL2Head eth.BlockID, l1Input []eth.BlockID) (out eth.BlockID, err error) {
	...
	for i, batch := range batches {
		...
		payload, err := d.addBlock(ctx, fc, attrs, true)
		if err != nil {
...
func (d *outputImpl) addBlock(ctx context.Context, fc l2.ForkchoiceState, attrs *l2.PayloadAttributes, safeHead bool) (*l2.ExecutionPayload, error) {
	...
	if safeHead {
		...
		fc.HeadBlockHash = payload.BlockHash // like this b/c of geth's setting w/ not showing safeblock properly
	} else {
		fc.HeadBlockHash = payload.BlockHash
	}
	_, err = d.l2.ForkchoiceUpdate(ctx, &fc, nil)
	if err != nil {
		return nil, fmt.Errorf("Could not make the new L2 block canonical via forkchoice: %w", err)
	}
	return payload, nil
}
