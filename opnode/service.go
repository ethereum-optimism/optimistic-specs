...
func NewConfig(ctx *cli.Context) (*node.Config, error) {
	...
	withdrawalContractAddress := common.BytesToAddress([]byte(ctx.GlobalString(flags.WithdrawalContractAddr.Name)))

	cfg := &node.Config{
		L1NodeAddr:             ctx.GlobalString(flags.L1NodeAddr.Name),
		L2EngineAddrs:          ctx.GlobalStringSlice(flags.L2EngineAddrs.Name),
		L2NodeAddr:             ctx.GlobalString(flags.L2NodeAddr.Name),
		Rollup:                 *rollupConfig,
